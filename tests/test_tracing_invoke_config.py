"""Unit tests for Langfuse Runnable config from host tracing + plugin metadata."""
...
def test_build_langfuse_llm_invoke_config_no_client() -> None:
    assert build_langfuse_llm_invoke_config(
        TracingExtras(enabled=True, langfuse_client=None),
        _PluginWithMetadata(),
    ) is None
...
def test_build_langfuse_llm_invoke_config_attaches_callback_and_metadata(
    ...
    client = MagicMock()
    te = TracingExtras(enabled=True, langfuse_client=client)
